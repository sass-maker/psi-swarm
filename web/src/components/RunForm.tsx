...
function QuickAndFullButtons({
  canRun,
  runTag,
  onStart,
}: {
  canRun: boolean;
  runTag: string | undefined;
  onStart: (plan: RunPlan) => void;
}) {
  return (
    ...
    <details className="group border-t border-[var(--color-border)] pt-4">
      ...
      <div className="space-y-4 pt-4">
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div>
            <label className="block text-sm text-[var(--color-dim)] mb-1.5" htmlFor="preset-group">
              Preset group
            </label>
            <select
              id="preset-group"
...
export function RunForm(props: RunFormProps) {
  ...
  return (
    ...
    >
      ...
      />
      <QuickAndFullButtons canRun={canRun} runTag={runTag} onStart={props.onStart} />
      <CustomSwarmSection props={props} canRun={canRun} runTag={runTag} onStart={props.onStart} />
    </form>
  );
}
