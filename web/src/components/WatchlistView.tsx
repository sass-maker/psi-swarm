...
function AddWatchlistForm({
  ...
  state: AddFormState;
  setState: (s: AddFormState) => void;
  onAdd: () => void;
}) {
  const { url, label, preset, baselineTag } = state;
  return (
    <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)] p-5">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <input
          value={preset}
          onChange={(e) => setState({ ...state, preset: e.target.value })}
          placeholder="Preset (mobile-mid)"
          className="rounded border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-sm"
        />
        <input
          value={baselineTag}
          onChange={(e) => setState({ ...state, baselineTag: e.target.value })}
  ...
  return (
    <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)] overflow-hidden">
      <div className="px-5 py-3 border-b border-[var(--color-border)] text-sm text-[var(--color-dim)]">
        {data.summary.regressed} regressed · {data.summary.improved} improved · {data.summary.stale}{' '}
        stale · {data.summary.missing} missing · {data.summary.stable} stable
      </div>
      {data.queue.length === 0 ? (
...
function useWatchlistActions(
  ...
  setData: (d: WatchlistResponse) => void
) {
  const refresh = async () => {
    if (!client) return;
    setError(null);
  ...
  const addUrl = async (form: AddFormState) => {
    if (!client || !form.url.trim()) return;
    setError(null);
    try {
      const res = await fetch(`${client.baseUrl}/api/watchlist`, {
        ...
        body: JSON.stringify({
          ...
          preset: form.preset,
          baselineTag: form.baselineTag.trim() || undefined,
        }),
      });
  ...
  return { refresh, addUrl, removeUrl };
}
...
export default function WatchlistView() {
  ...
  const { refresh, addUrl, removeUrl } = useWatchlistActions(client, setError, setData);

  const handleAdd = async () => {
    await addUrl(form);
    setForm({ ...form, url: '', label: '', baselineTag: '' });
  };
  ...
  return (
    <div className="space-y-6">
      <WatchlistHeader health={health} data={data} onRefresh={() => void refresh()} />
      {error && <p className="text-sm text-[var(--color-poor)]">{error}</p>}
      <AddWatchlistForm state={form} setState={setForm} onAdd={() => void handleAdd()} />
      {data && <WatchlistQueue data={data} onRemove={(t) => void removeUrl(t)} />}
    </div>
  );
}
